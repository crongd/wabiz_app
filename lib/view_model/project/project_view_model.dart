...
import 'package:wabiz_app/repository/project/project_repository.dart';
...
@riverpod
Future<ProjectModel> fetchProjectById(Ref ref, String id) async {
...
class ProjectViewModel extends _$ProjectViewModel {
  ...
  Future<bool> createProject(ProjectItemModel body) async {
    final result = await ref.watch(projectRepositoryProvider).createProject(body);
  ...
  Future<bool> createProjectReward(String id, RewardItemModel body) async {
    final result = await ref.watch(projectRepositoryProvider).createdProjectReward(id, body);
    if (result?.status == 'ok') {
      return true;
    }
    return false;
  ...
  Future<bool> updateProjectOpenState(String id, ProjectItemModel body) async {
    final result = await ref.watch(projectRepositoryProvider).updateProjectOpenState(id, body);
    if (result?.status == 'ok') {
      return true;
    }
    return false;
